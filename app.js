...
function renderMembers(){
  ...
  $("membersList").innerHTML = list.length ? list.map(m => `
    <div class="member"><div class="avatar"></div><div><strong>${m.gamertag||"Operador"}</strong><p>${m.rank||"Recruit"} · ${m.region||"Global"}<br>Discord: ${m.discord||"No agregado"}</p></div></div>
...
function renderRanking(){
  ...
  $("rankingList").innerHTML = list.length ? list.map((m,i)=>`
    <div class="row"><span>${i+1}</span><strong>${m.gamertag||"Operador"}</strong><span>${m.region||"🌐"}</span><span>${m.kd||"0.00"}</span><span>${m.wins||"0"}</span></div>
...
window.acceptRequest = async id => {
  ...
  await push(ref(db,"members"), {gamertag:r.gamertag, rank:"Recruit", region:r.region, discord:r.discord||"", kd:r.kd||"0.00", wins:"0", emailPrivate:r.email, activision:r.activision||"", createdAt:new Date().toISOString()});
